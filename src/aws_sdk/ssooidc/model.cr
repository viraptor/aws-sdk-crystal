module AwsSdk
  module SSOOIDC
    module Model
      ...
      CREATE_TOKEN_WITH_IAM = OperationModel.new(
        name: "CreateTokenWithIAM",
        http_method: "POST",
        request_uri: "/token?aws_iam=t"
      )

      REGISTER_CLIENT = OperationModel.new(
        name: "RegisterClient",
        http_method: "POST",
        request_uri: "/client/register"
      )

      START_DEVICE_AUTHORIZATION = OperationModel.new(
        name: "StartDeviceAuthorization",
        http_method: "POST",
        request_uri: "/device_authorization"
      )
    end
  end
end
