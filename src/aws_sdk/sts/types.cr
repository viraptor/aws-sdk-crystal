  ...
  module STS
    module Types
      ...
      struct AssumeRoleRequest
        ...
        # An identifier for the assumed role session. Use the role session name to uniquely identify a session
        # when the same role is assumed by different principals or for different reasons. In cross-account
        # scenarios, the role session name is visible to, and can be logged by the account that owns the role.
        # The role session name is also used in the ARN of the assumed role principal. This means that
        # subsequent cross-account API requests that use the temporary security credentials will expose the
        # role session name to the external account in their CloudTrail logs. For security purposes,
        # administrators can view this field in CloudTrail logs to help identify who performed an action in
        # Amazon Web Services. Your administrator might require that you specify your user name as the session
        # name when you assume the role. For more information, see sts:RoleSessionName . The regex used to
        ...
        # minutes) up to the maximum session duration set for the role. The maximum session duration setting
        # can have a value from 1 hour to 12 hours. If you specify a value higher than this setting or the
        # administrator setting (whichever is lower), the operation fails. For example, if you specify a
        # session duration of 12 hours, but your administrator set the maximum session duration to 6 hours,
        # your operation fails. Role chaining limits your Amazon Web Services CLI or Amazon Web Services API
        # role session to a maximum of one hour. When you use the AssumeRole API operation to assume a role,
        # you can specify the duration of your role session with the DurationSeconds parameter. You can
        # specify a parameter value of up to 43200 seconds (12 hours), depending on the maximum session
        # duration setting for your role. However, if you assume a role using role chaining and provide a
        # DurationSeconds parameter value greater than one hour, the operation fails. To learn how to view the
        # maximum value for your role, see Update the maximum session duration for a role . By default, the
        # value is set to 3600 seconds. The DurationSeconds parameter is separate from the duration of a
        # console session that you might request using the returned credentials. The request to the federation
        ...
        # or account number. A cross-account role is usually set up to trust everyone in an account.
        # Therefore, the administrator of the trusting account might send an external ID to the administrator
        # of the trusted account. That way, only someone with the ID can assume the role, rather than everyone
        # in the account. For more information about the external ID, see How to Use an External ID When
        # Granting Access to Your Amazon Web Services Resources to a Third Party in the IAM User Guide . The
        ...
        # An IAM policy in JSON format that you want to use as an inline session policy. This parameter is
        # optional. Passing policies to this operation returns new temporary credentials. The resulting
        # session's permissions are the intersection of the role's identity-based policy and the session
        # policies. You can use the role's temporary credentials in subsequent Amazon Web Services API calls
        # to access resources in the account that owns the role. You cannot use session policies to grant more
        # permissions than those allowed by the identity-based policy of the role that is being assumed. For
        # more information, see Session Policies in the IAM User Guide . The plaintext that you use for both
        # inline and managed session policies can't exceed 2,048 characters. The JSON policy characters can be
        # any ASCII character from the space character to the end of the valid character list (\u0020 through
        # \u00FF). It can also include the tab (\u0009), linefeed (\u000A), and carriage return (\u000D)
        # characters. An Amazon Web Services conversion compresses the passed inline session policy, managed
        # policy ARNs, and session tags into a packed binary format that has a separate limit. Your request
        ...
        # policies. The policies must exist in the same account as the role. This parameter is optional. You
        # can provide up to 10 managed policy ARNs. However, the plaintext that you use for both inline and
        # managed session policies can't exceed 2,048 characters. For more information about ARNs, see Amazon
        # Resource Names (ARNs) and Amazon Web Services Service Namespaces in the Amazon Web Services General
        # Reference. An Amazon Web Services conversion compresses the passed inline session policy, managed
        # policy ARNs, and session tags into a packed binary format that has a separate limit. Your request
        # can fail for this limit even if your plaintext meets the other requirements. The PackedPolicySize
        # response element indicates by percentage how close the policies and tags for your request are to the
        # upper size limit. Passing policies to this operation returns new temporary credentials. The
        ...
        # context assertion is signed and encrypted by Amazon Web Services STS. The following is an example of
        ...
        # The source identity specified by the principal that is calling the AssumeRole operation. The source
        # identity value persists across chained role sessions. You can require users to specify a source
        # identity when they assume a role. You do this by using the sts:SourceIdentity condition key in a
        # role trust policy. You can use source identity information in CloudTrail logs to determine who took
        # actions with a role. You can use the aws:SourceIdentity condition key to further control access to
        # Amazon Web Services resources based on the value of source identity. For more information about
        # using source identity, see Monitor and control actions taken with assumed roles in the IAM User
        # Guide . The regex used to validate this parameter is a string of characters consisting of upper- and
        ...
        # Sessions in the IAM User Guide . This parameter is optional. You can pass up to 50 session tags. The
        # plaintext session tag keys can’t exceed 128 characters, and the values can’t exceed 256 characters.
        # For these and additional limits, see IAM and STS Character Limits in the IAM User Guide . An Amazon
        # Web Services conversion compresses the passed inline session policy, managed policy ARNs, and
        # session tags into a packed binary format that has a separate limit. Your request can fail for this
        ...
        # limit. You can pass a session tag with the same key as a tag that is already attached to the role.
        # When you do, session tags override a role tag with the same key. Tag key–value pairs are not case
        # sensitive, but case is preserved. This means that you cannot have separate Department and department
        # tag keys. Assume that the role has the Department = Marketing tag and you pass the department =
        # engineering session tag. Department and department are not saved as separate tags, and the session
        # tag passed in the request takes precedence over the role tag. Additionally, if you used temporary
        # credentials to perform this operation, the new session inherits any transitive session tags from the
        ...
        # (In other words, if the policy includes a condition that tests for MFA). If the role being assumed
        ...
        # information, see Chaining Roles with Session Tags in the IAM User Guide . This parameter is
      ...
      struct AssumeRoleResponse
        ...
        # The source identity specified by the principal that is calling the AssumeRole operation. You can
        # require users to specify a source identity when they assume a role. You do this by using the
        ...
        # condition key to further control access to Amazon Web Services resources based on the value of
        # source identity. For more information about using source identity, see Monitor and control actions
        ...
        def initialize(
          @assumed_role_user : Types::AssumedRoleUser? = nil,
      ...
      struct AssumeRoleWithSAMLRequest
        ...
        # specify for the DurationSeconds parameter, or until the time specified in the SAML authentication
        # response's SessionNotOnOrAfter value, whichever is shorter. You can provide a DurationSeconds value
        # from 900 seconds (15 minutes) up to the maximum session duration setting for the role. This setting
        # can have a value from 1 hour to 12 hours. If you specify a value higher than this setting, the
        # operation fails. For example, if you specify a session duration of 12 hours, but your administrator
        # set the maximum session duration to 6 hours, your operation fails. To learn how to view the maximum
        # value for your role, see View the Maximum Session Duration Setting for a Role in the IAM User Guide
        # . By default, the value is set to 3600 seconds. The DurationSeconds parameter is separate from the
        # duration of a console session that you might request using the returned credentials. The request to
        ...
        # any ASCII character from the space character to the end of the valid character list (\u0020 through
        # \u00FF). It can also include the tab (\u0009), linefeed (\u000A), and carriage return (\u000D)
        # characters. For more information about role session permissions, see Session policies . An Amazon
        # Web Services conversion compresses the passed inline session policy, managed policy ARNs, and
        # session tags into a packed binary format that has a separate limit. Your request can fail for this
        # limit even if your plaintext meets the other requirements. The PackedPolicySize response element
        # indicates by percentage how close the policies and tags for your request are to the upper size
      ...
      struct AssumeRoleWithSAMLResponse
        ...
        # A hash value based on the concatenation of the following: The Issuer response value. The Amazon Web
        # Services account ID. The friendly name (the last part of the ARN) of the SAML provider in IAM. The
        ...
        # The value in the SourceIdentity attribute in the SAML assertion. The source identity value persists
        # across chained role sessions. You can require users to set a source identity value when they assume
        # a role. You do this by using the sts:SourceIdentity condition key in a role trust policy. That way,
        # actions that are taken with the role are associated with that user. After the source identity is
        # set, the value cannot be changed. It is present in the request for all actions that are taken by the
        # role and persists across chained role sessions. You can configure your SAML identity provider to use
        # an attribute associated with your users, like user name or email, as the source identity when
        # calling AssumeRoleWithSAML . You do this by adding an attribute to the SAML assertion. For more
        # information about using source identity, see Monitor and control actions taken with assumed roles in
        ...
        # The format of the name ID, as defined by the Format attribute in the NameID element of the SAML
        # assertion. Typical examples of the format are transient or persistent . If the format includes the
      ...
      struct AssumeRoleWithWebIdentityRequest
        ...
        # The Amazon Resource Name (ARN) of the role that the caller is assuming. Additional considerations
        # apply to Amazon Cognito identity pools that assume cross-account IAM roles . The trust policies of
        # these roles must accept the cognito-identity.amazonaws.com service principal and must contain the
        # cognito-identity.amazonaws.com:aud condition key to restrict role assumption to users from your
        # intended identity pools. A policy that trusts Amazon Cognito identity pools without this condition
        ...
        # of the ARN and assumed role ID in the AssumedRoleUser response element. For security purposes,
        # administrators can view this field in CloudTrail logs to help identify who performed an action in
        # Amazon Web Services. Your administrator might require that you specify your user name as the session
        # name when you assume the role. For more information, see sts:RoleSessionName . The regex used to
        # validate this parameter is a string of characters consisting of upper- and lower-case alphanumeric
        ...
        # Your application must get this token by authenticating the user who is using your application with a
        ...
        # The duration, in seconds, of the role session. The value can range from 900 seconds (15 minutes) up
        # to the maximum session duration setting for the role. This setting can have a value from 1 hour to
        # 12 hours. If you specify a value higher than this setting, the operation fails. For example, if you
        # specify a session duration of 12 hours, but your administrator set the maximum session duration to 6
        # hours, your operation fails. To learn how to view the maximum value for your role, see View the
        # Maximum Session Duration Setting for a Role in the IAM User Guide . By default, the value is set to
        # 3600 seconds. The DurationSeconds parameter is separate from the duration of a console session that
        # you might request using the returned credentials. The request to the federation endpoint for a
      ...
      struct AssumeRoleWithWebIdentityResponse
        ...
        # provider. You can require users to set a source identity value when they assume a role. You do this
        # by using the sts:SourceIdentity condition key in a role trust policy. That way, actions that are
        ...
        # persists across chained role sessions. You can configure your identity provider to use an attribute
        # associated with your users, like user name or email, as the source identity when calling
        # AssumeRoleWithWebIdentity . You do this by adding a claim to the JSON web token. To learn more about
        # OIDC tokens and claims, see Using Tokens with User Pools in the Amazon Cognito Developer Guide . For
        # more information about using source identity, see Monitor and control actions taken with assumed
        ...
        # with the WebIdentityToken that was submitted with the AssumeRoleWithWebIdentity call. The identifier
        ...
        def initialize(
          ...
          @provider : String? = nil,
          @source_identity : String? = nil,
          @subject_from_web_identity_token : String? = nil
        )
        end
      end
      ...
      struct AssumeRootResponse
        ...
        # The source identity specified by the principal that is calling the AssumeRoot operation. You can use
        # the aws:SourceIdentity condition key to control access based on the value of source identity. For
        # more information about using source identity, see Monitor and control actions taken with assumed
        # roles in the IAM User Guide . The regex used to validate this parameter is a string of characters
        # consisting of upper- and lower-case alphanumeric characters with no spaces. You can also include
        # underscores or any of the following characters: =,.@-
      ...
      struct GetFederationTokenRequest
        ...
        # An IAM policy in JSON format that you want to use as an inline session policy. You must pass an
        # inline or managed session policy to this operation. You can pass a single JSON policy document to
        # use as an inline session policy. You can also specify up to 10 managed policy Amazon Resource Names
        # (ARNs) to use as managed session policies. This parameter is optional. However, if you do not pass
        # any session policies, then the resulting federated user session has no permissions. When you pass
        # session policies, the session permissions are the intersection of the IAM user policies and the
        # session policies that you pass. This gives you a way to further restrict the permissions for a
        # federated user. You cannot use session policies to grant more permissions than those that are
        # defined in the permissions policy of the IAM user. For more information, see Session Policies in the
        # IAM User Guide . The resulting credentials can be used to access a resource that has a
        # resource-based policy. If that policy specifically references the federated user session in the
        # Principal element of the policy, the session has the permissions allowed by the policy. These
        # permissions are granted in addition to the permissions that are granted by the session policies. The
        # plaintext that you use for both inline and managed session policies can't exceed 2,048 characters.
        # The JSON policy characters can be any ASCII character from the space character to the end of the
        # valid character list (\u0020 through \u00FF). It can also include the tab (\u0009), linefeed
        ...
        # federated access. You must pass an inline or managed session policy to this operation. You can pass
        # a single JSON policy document to use as an inline session policy. You can also specify up to 10
        # managed policy Amazon Resource Names (ARNs) to use as managed session policies. The plaintext that
        # you use for both inline and managed session policies can't exceed 2,048 characters. You can provide
        # up to 10 managed policy ARNs. For more information about ARNs, see Amazon Resource Names (ARNs) and
        # Amazon Web Services Service Namespaces in the Amazon Web Services General Reference. This parameter
        # is optional. However, if you do not pass any session policies, then the resulting federated user
        # session has no permissions. When you pass session policies, the session permissions are the
        # intersection of the IAM user policies and the session policies that you pass. This gives you a way
        # to further restrict the permissions for a federated user. You cannot use session policies to grant
        # more permissions than those that are defined in the permissions policy of the IAM user. For more
        # information, see Session Policies in the IAM User Guide . The resulting credentials can be used to
        ...
        # allowed by the policy. These permissions are granted in addition to the permissions that are granted
        # by the session policies. An Amazon Web Services conversion compresses the passed inline session
        # policy, managed policy ARNs, and session tags into a packed binary format that has a separate limit.
        ...
        # information about session tags, see Passing Session Tags in STS in the IAM User Guide . This
        # parameter is optional. You can pass up to 50 session tags. The plaintext session tag keys can’t
        ...
        # compresses the passed inline session policy, managed policy ARNs, and session tags into a packed
        # binary format that has a separate limit. Your request can fail for this limit even if your plaintext
        # meets the other requirements. The PackedPolicySize response element indicates by percentage how
        # close the policies and tags for your request are to the upper size limit. You can pass a session tag
        # with the same key as a tag that is already attached to the user you are federating. When you do,
        # session tags override a user tag with the same key. Tag key–value pairs are not case sensitive, but
        # case is preserved. This means that you cannot have separate Department and department tag keys.
      ...
      struct GetSessionTokenRequest
        ...
        # user sessions range from 900 seconds (15 minutes) to 129,600 seconds (36 hours), with 43,200 seconds
        ...
        # You can find the device for an IAM user by going to the Amazon Web Services Management Console and
      ...
      struct GetWebIdentityTokenRequest
        ...
        # The duration, in seconds, for which the JSON Web Token (JWT) will remain valid. The value can range
