module Aws
  module EventBridge
    class Client
      ...
      # API destinations do not support private destinations, such as interface VPC endpoints. For more
      # information, see API destinations in the EventBridge User Guide .
      ...
      # bus as well. For more information, see Encrypting archives in the Amazon EventBridge User Guide .
      def create_archive(
        archive_name : String,
        ...
        input = Types::CreateArchiveRequest.new(archive_name: archive_name, event_source_arn: event_source_arn, description: description, event_pattern: event_pattern, kms_key_identifier: kms_key_identifier, retention_days: retention_days)
      ...
      # authorization with an API destination HTTP endpoint. For more information, see Connections for
      # endpoint targets in the Amazon EventBridge User Guide .
      def create_connection(
        ...
        input = Types::CreateConnectionRequest.new(auth_parameters: auth_parameters, authorization_type: authorization_type, name: name, description: description, invocation_connectivity_parameters: invocation_connectivity_parameters, kms_key_identifier: kms_key_identifier)
      ...
      # regional-fault tolerant. To do this, you define a primary and secondary Region with event buses in
      # each Region. You also create a Amazon Route 53 health check that will tell EventBridge to route
      # events to the secondary Region when an "unhealthy" state is encountered and events will be routed
      ...
      def create_endpoint(
        ...
        input = Types::CreateEndpointRequest.new(event_buses: event_buses, name: name, routing_config: routing_config, description: description, replication_config: replication_config, role_arn: role_arn)
      ...
      def create_event_bus(
        ...
        input = Types::CreateEventBusRequest.new(name: name, dead_letter_config: dead_letter_config, description: description, event_source_name: event_source_name, kms_key_identifier: kms_key_identifier, log_config: log_config, tags: tags)
      ...
      # partner system. The event_name must be unique across all Amazon Web Services customers. This is
      # because the event source is a shared resource between the partner and customer accounts, and each
      # partner event source unique in the partner account. The combination of event_namespace and
      # event_name should help Amazon Web Services customers decide whether to create an event bus to
      ...
      def delete_endpoint(
        ...
        input = Types::DeleteEndpointRequest.new(name: name)
      ...
      # Get the information about an existing global endpoint. For more information about global endpoints,
      # see Making applications Regional-fault tolerant with global endpoints and event replication in the
      # Amazon EventBridge User Guide .
      def describe_endpoint(
        name : String,
        home_region : String? = nil
      ) : Types::DescribeEndpointResponse
        input = Types::DescribeEndpointRequest.new(name: name, home_region: home_region)
      ...
      # List the global endpoints associated with this account. For more information about global endpoints,
      # see Making applications Regional-fault tolerant with global endpoints and event replication in the
      # Amazon EventBridge User Guide .
      def list_endpoints(
        home_region : String? = nil,
        ...
        input = Types::ListEndpointsRequest.new(home_region: home_region, max_results: max_results, name_prefix: name_prefix, next_token: next_token)
      ...
      # invoke a specific target in your account. The maximum number of results per page for requests is
      # 100.
      ...
      # to match to the rule names. The maximum number of results per page for requests is 100. ListRules
      # does not list the targets of a rule. To see the targets associated with a rule, use
      # ListTargetsByRule .
      def list_rules(
        event_bus_name : String? = nil,
        limit : Int32? = nil,
      ...
      # Lists the targets assigned to the specified rule. The maximum number of results per page for
      # requests is 100.
      ...
      # Sends custom events to Amazon EventBridge so that they can be matched to rules. You can batch
      # multiple event entries into one request for efficiency. However, the total entry size must be less
      # than 256KB. You can calculate the entry size before you send the events. For more information, see
      # Calculating PutEvents event entry size in the Amazon EventBridge User Guide . PutEvents accepts the
      # data in JSON format. For the JSON number (integer) data type, the constraints are: a minimum value
      # of -9,223,372,036,854,775,808 and a maximum value of 9,223,372,036,854,775,807. PutEvents will only
      # process nested JSON up to 1000 levels deep.
      def put_events(
        entries : Array(Types::PutEventsRequestEntry),
        endpoint_id : String? = nil
      ) : Types::PutEventsResponse
        input = Types::PutEventsRequest.new(entries: entries, endpoint_id: endpoint_id)
        put_events(input)
      end
      ...
      # customers do not use this operation. For information on calculating event batch size, see
      # Calculating EventBridge PutEvents event entry size in the EventBridge User Guide .
      ...
      # organization to put events to the specified event bus . Amazon EventBridge rules in your account are
      # triggered by these events arriving to an event bus in your account. For another account to send
      # events to your account, that external account must have an EventBridge rule with your account's
      # event bus as a target. To enable multiple Amazon Web Services accounts to put events to your event
      # bus, run PutPermission once for each of these accounts. Or, if all the accounts are members of the
      # same Amazon Web Services organization, you can run PutPermission once specifying Principal as "*"
      # and specifying the Amazon Web Services organization ID in Condition , to grant permissions to all
      # accounts in that organization. If you grant permissions using an organization, then accounts in that
      # organization must specify a RoleArn with proper permissions when they use PutTarget to add your
      # account's event bus as a target. For more information, see Sending and Receiving Events Between
      # Amazon Web Services Accounts in the Amazon EventBridge User Guide . The permission policy on the
      # event bus cannot exceed 10 KB in size.
      def put_permission(
        action : String? = nil,
      ...
      # more information, see Managing Your Costs with Budgets . To create a rule that filters for
      # management events from Amazon Web Services services, see Receiving read-only management events from
      # Amazon Web Services services in the EventBridge User Guide .
      def put_rule(
        name : String,
        description : String? = nil,
        event_bus_name : String? = nil,
      ...
      # associated with the rule. Targets are the resources that are invoked when a rule is triggered. The
      # maximum number of entries per request is 10. Each rule can have up to five (5) targets associated
      # with it at one time. For a list of services you can configure as targets for events, see EventBridge
      # targets in the Amazon EventBridge User Guide . Creating rules with built-in targets is supported
      # only in the Amazon Web Services Management Console. The built-in targets are: Amazon EBS
      # CreateSnapshot API call Amazon EC2 RebootInstances API call Amazon EC2 StopInstances API call Amazon
      # EC2 TerminateInstances API call For some target types, PutTargets provides target-specific
      # parameters. If the target is a Kinesis data stream, you can optionally specify which shard the event
      # goes to by using the KinesisParameters argument. To invoke a command on multiple EC2 instances with
      # one rule, you can use the RunCommandParameters field. To be able to make API calls against the
      # resources that you own, Amazon EventBridge needs the appropriate permissions: For Lambda and Amazon
      # SNS resources, EventBridge relies on resource-based policies. For EC2 instances, Kinesis Data
      # Streams, Step Functions state machines and API Gateway APIs, EventBridge relies on IAM roles that
      # you specify in the RoleARN argument in PutTargets . For more information, see Authentication and
      # Access Control in the Amazon EventBridge User Guide . If another Amazon Web Services account is in
      # the same region and has granted you permission (using PutPermission ), you can send events to that
      # account. Set that account's event bus as a target of the rules in your account. To send the matched
      # events to the other account, specify that account's event bus as the Arn value when you run
      # PutTargets . If your account sends events to another account, your account is charged for each sent
      # event. Each event sent to another account is charged as a custom event. The account receiving the
      # event is not charged. For more information, see Amazon EventBridge Pricing . Input , InputPath , and
      # InputTransformer are not available with PutTarget if the target is an event bus of a different
      # Amazon Web Services account. If you are setting the event bus of another account as the target, and
      # that account granted permission to your account through an organization instead of directly by the
      # account ID, then you must specify a RoleArn with proper permissions in the Target structure. For
      # more information, see Sending and Receiving Events Between Amazon Web Services Accounts in the
      # Amazon EventBridge User Guide . If you have an IAM role on a cross-account event bus target, a
      # PutTargets call without a role on the same target (same Id and Arn ) will not remove the role. For
      # more information about enabling cross-account events, see PutPermission . Input , InputPath , and
      # InputTransformer are mutually exclusive and optional parameters of a target. When a rule is
      # triggered due to a matched event: If none of the following arguments are specified for a target,
      # then the entire event is passed to the target in JSON format (unless the target is Amazon EC2 Run
      # Command or Amazon ECS task, in which case nothing from the event is passed to the target). If Input
      # is specified in the form of valid JSON, then the matched event is overridden with this constant. If
      # InputPath is specified in the form of JSONPath (for example, $.detail ), then only the part of the
      # event specified in the path is passed to the target (for example, only the detail part of the event
      # is passed). If InputTransformer is specified, then one or more specified JSONPaths are extracted
      # from the event and used as values in a template that you specify as the input to the target. When
      # you specify InputPath or InputTransformer , you must use JSON dot notation, not bracket notation.
      # When you add targets to a rule and the associated rule triggers soon after, new or updated targets
      # might not be immediately invoked. Allow a short period of time for changes to take effect. This
      # action can partially fail if too many requests are made at the same time. If that happens,
      # FailedEntryCount is non-zero in the response and each entry in FailedEntries provides the ID of the
      ...
      # no longer be invoked. A successful execution of RemoveTargets doesn't guarantee all targets are
      # removed from the rule, it means that the target(s) listed in the request are removed. When you
      # remove a target, when the associated rule triggers, removed targets might continue to be invoked.
      # Allow a short period of time for changes to take effect. This action can partially fail if too many
      # requests are made at the same time. If that happens, FailedEntryCount is non-zero in the response
      # and each entry in FailedEntries provides the ID of the failed target and the error code. The maximum
      # number of entries per request is 10.
      def remove_targets(
        ids : Array(String),
        rule : String,
        event_bus_name : String? = nil,
      ...
      # Removes one or more tags from the specified EventBridge resource. In Amazon EventBridge, rules and
      ...
      def update_archive(
        ...
        input = Types::UpdateArchiveRequest.new(archive_name: archive_name, description: description, event_pattern: event_pattern, kms_key_identifier: kms_key_identifier, retention_days: retention_days)
      ...
      def update_connection(
        ...
        input = Types::UpdateConnectionRequest.new(name: name, auth_parameters: auth_parameters, authorization_type: authorization_type, description: description, invocation_connectivity_parameters: invocation_connectivity_parameters, kms_key_identifier: kms_key_identifier)
      ...
      def update_endpoint(
        ...
        input = Types::UpdateEndpointRequest.new(name: name, description: description, event_buses: event_buses, replication_config: replication_config, role_arn: role_arn, routing_config: routing_config)
      ...
      def update_event_bus(
        ...
        log_config : Types::LogConfig? = nil,
        name : String? = nil
      ) : Types::UpdateEventBusResponse
        input = Types::UpdateEventBusRequest.new(dead_letter_config: dead_letter_config, description: description, kms_key_identifier: kms_key_identifier, log_config: log_config, name: name)
        update_event_bus(input)
      end

      def update_event_bus(input : Types::UpdateEventBusRequest) : Types::UpdateEventBusResponse
        request = Protocol::JsonRpc.build_request(Model::UPDATE_EVENT_BUS, input, endpoint)
        request = request.with_headers(endpoint_headers)
        response = runtime.execute(request)
        raise Protocol::JsonRpc.parse_error(response) if response.status >= 300
        Protocol::JsonRpc.parse_response(response, Types::UpdateEventBusResponse, "UpdateEventBus")
      end
    end
  end
end
