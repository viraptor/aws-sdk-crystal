  ...
  module KMS
    module Types
      ...
      struct CancelKeyDeletionRequest
        ...
        # Identifies the KMS key whose deletion is being canceled. Specify the key ID or key ARN of the KMS
      ...
      struct CreateAliasRequest
        ...
        # alias/ExampleAlias . Do not include confidential or sensitive information in this field. This field
        # may be displayed in plaintext in CloudTrail logs and other output. The AliasName value must be
        ...
        # Amazon Web Services Region. A valid key ID is required. If you supply a null or empty string value,
        # this operation returns an error. For help finding the key ID and ARN, see Find the key ID and key
        # ARN in the Key Management Service Developer Guide . Specify the key ID or key ARN of the KMS key.
      ...
      struct CreateCustomKeyStoreRequest
        ...
        # Specifies the type of custom key store. The default value is AWS_CLOUDHSM . For a custom key store
        ...
        # stores with a CustomKeyStoreType of AWS_CLOUDHSM . Enter the password of the kmsuser crypto user
        # (CU) account in the specified CloudHSM cluster. KMS logs into the cluster as this user to manage key
        ...
        # Specifies an authentication credential for the external key store proxy (XKS proxy). This parameter
        # is required for all custom key stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE . The
        # XksProxyAuthenticationCredential has two required elements: RawSecretAccessKey , a secret key, and
        # AccessKeyId , a unique identifier for the RawSecretAccessKey . For character requirements, see
        # XksProxyAuthenticationCredentialType . KMS uses this authentication credential to sign requests to
        # the external key store proxy on your behalf. This credential is unrelated to Identity and Access
        # Management (IAM) and Amazon Web Services credentials. This parameter doesn't set or change the
        ...
        # custom key stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE . If the external key store proxy
        # uses a public endpoint, specify PUBLIC_ENDPOINT . If the external key store proxy uses a Amazon VPC
        # endpoint service for communication with KMS, specify VPC_ENDPOINT_SERVICE . For help making this
        # choice, see Choosing a connectivity option in the Key Management Service Developer Guide . An Amazon
        # VPC endpoint service keeps your communication with KMS in a private address space entirely within
        # Amazon Web Services, but it requires more configuration, including establishing a Amazon VPC with
        # multiple subnets, a VPC endpoint service, a network load balancer, and a verified private DNS name.
        # A public endpoint is simpler to set up, but it might be slower and might not fulfill your security
        # requirements. You might consider testing with a public endpoint, and then establishing a VPC
        ...
        # This parameter is required for custom key stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE .
        # The protocol must be HTTPS. KMS communicates on port 443. Do not specify the port in the
        # XksProxyUriEndpoint value. For external key stores with XksProxyConnectivity value of
        # VPC_ENDPOINT_SERVICE , specify https:// followed by the private DNS name of the VPC endpoint
        # service. For external key stores with PUBLIC_ENDPOINT connectivity, this endpoint must be reachable
        # before you create the custom key store. KMS connects to the external key store proxy while creating
        # the custom key store. For external key stores with VPC_ENDPOINT_SERVICE connectivity, KMS connects
        # when you call the ConnectCustomKeyStore operation. The value of this parameter must begin with
        # https:// . The remainder can contain upper and lower case letters (A-Z and a-z), numbers (0-9), dots
        ...
        # Services account and Region. An external key store with PUBLIC_ENDPOINT connectivity cannot use the
        # same XksProxyUriEndpoint value as an external key store with VPC_ENDPOINT_SERVICE connectivity in
        # this Amazon Web Services Region. Each external key store with VPC_ENDPOINT_SERVICE connectivity must
        ...
        # stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE . The value must start with / and must end
        # with /kms/xks/v1 where v1 represents the version of the KMS external key store proxy API. This path
        ...
        # VPC_ENDPOINT_SERVICE . The Amazon VPC endpoint service must fulfill all requirements for use with an
      ...
      struct CreateGrantRequest
        ...
        # The identity that gets the permissions specified in the grant. To specify the grantee principal, use
        ...
        # Services account, you must use the key ARN. For example: Key ID:
        ...
        # A list of operations that the grant permits. This list must include only operations that are
        # permitted in a grant. Also, the operation must be supported on the KMS key. For example, you cannot
        # create a grant for a symmetric encryption KMS key that allows the Sign operation, or a grant for an
        ...
        # Specifies a grant constraint. Do not include confidential or sensitive information in this field.
        # This field may be displayed in plaintext in CloudTrail logs and other output. KMS supports the
        # EncryptionContextEquals and EncryptionContextSubset grant constraints, which allow the permissions
        # in the grant only when the encryption context in the request matches ( EncryptionContextEquals ) or
        # includes ( EncryptionContextSubset ) the encryption context specified in the constraint. The
        # encryption context grant constraints are supported only on grant operations that include an
        # EncryptionContext parameter, such as cryptographic operations on symmetric encryption KMS keys.
        # Grants with grant constraints can include the DescribeKey and RetireGrant operations, but the
        # constraint doesn't apply to these operations. If a grant with a grant constraint includes the
        # CreateGrant operation, the constraint requires that any grants created with the CreateGrant
        # permission have an equally strict or stricter encryption context constraint. You cannot use an
        # encryption context grant constraint for cryptographic operations with asymmetric KMS keys or HMAC
        # KMS keys. Operations with these keys don't support an encryption context. Each constraint value can
        # include up to 8 encryption context pairs. The encryption context value in each constraint cannot
        ...
        # when retrying this request. Do not include confidential or sensitive information in this field. This
        # field may be displayed in plaintext in CloudTrail logs and other output. When this value is absent,
        # all CreateGrant requests result in a new grant with a unique GrantId even if all the supplied
        # parameters are identical. This can result in unintended duplicates when you retry the CreateGrant
        # request. When this value is present, you can retry a CreateGrant request with identical parameters;
        ...
        # The principal that has permission to use the RetireGrant operation to retire the grant. To specify
        # the principal, use the Amazon Resource Name (ARN) of an Amazon Web Services principal. Valid
        # principals include Amazon Web Services accounts, IAM users, IAM roles, federated users, and assumed
        # role users. For help with the ARN syntax for a principal, see IAM ARNs in the Identity and Access
      ...
      struct CreateKeyRequest
        ...
        # Skips ("bypasses") the key policy lockout safety check. The default value is false. Setting this
        # value to true increases the risk that the KMS key becomes unmanageable. Do not set this value to
        ...
        # Creates the KMS key in the specified custom key store . The ConnectionState of the custom key store
        # must be CONNECTED . To find the CustomKeyStoreID and ConnectionState use the DescribeCustomKeyStores
        # operation. This parameter is valid only for symmetric encryption KMS keys in a single Region. You
        # cannot create any other type of KMS key in a custom key store. When you create a KMS key in an
        # CloudHSM key store, KMS generates a non-exportable 256-bit symmetric key in its associated CloudHSM
        ...
        # appropriate for a task. The default value is an empty string (no description). Do not include
        # confidential or sensitive information in this field. This field may be displayed in plaintext in
        ...
        # with a 256-bit AES-GCM key that is used for encryption and decryption, except in China Regions,
        # where it creates a 128-bit symmetric key that uses SM4 encryption. For a detailed description of all
        # supported key specs, see Key spec reference in the Key Management Service Developer Guide . The
        # KeySpec determines whether the KMS key contains a symmetric key or an asymmetric key pair. It also
        # determines the algorithms that the KMS key supports. You can't change the KeySpec after the KMS key
        # is created. To further restrict the algorithms that can be used with the KMS key, use a condition
        # key in its key policy or IAM policy. For more information, see kms:EncryptionAlgorithm ,
        # kms:MacAlgorithm , kms:KeyAgreementAlgorithm , or kms:SigningAlgorithm in the Key Management Service
        # Developer Guide . Amazon Web Services services that are integrated with KMS use symmetric encryption
        # KMS keys to protect your data. These services do not support asymmetric KMS keys or HMAC KMS keys.
        # KMS supports the following key specs for KMS keys: Symmetric encryption key (default)
        # SYMMETRIC_DEFAULT HMAC keys (symmetric) HMAC_224 HMAC_256 HMAC_384 HMAC_512 Asymmetric RSA key pairs
        # (encryption and decryption -or- signing and verification) RSA_2048 RSA_3072 RSA_4096 Asymmetric
        # NIST-standard elliptic curve key pairs (signing and verification -or- deriving shared secrets)
        # ECC_NIST_P256 (secp256r1) ECC_NIST_P384 (secp384r1) ECC_NIST_P521 (secp521r1) ECC_NIST_EDWARDS25519
        # (ed25519) - signing and verification only Note: For ECC_NIST_EDWARDS25519 KMS keys, the
        # ED25519_SHA_512 signing algorithm requires MessageType:RAW , while ED25519_PH_SHA_512 requires
        # MessageType:DIGEST . These message types cannot be used interchangeably. Other asymmetric elliptic
        # curve key pairs (signing and verification) ECC_SECG_P256K1 (secp256k1), commonly used for
        ...
        # ENCRYPT_DECRYPT . This parameter is optional when you are creating a symmetric encryption KMS key;
        # otherwise, it is required. You can't change the KeyUsage value after the KMS key is created. Each
        # KMS key can have only one key usage. This follows key usage best practices according to NIST SP
        # 800-57 Recommendations for Key Management , section 5.2, Key usage. Select only one valid value. For
        # symmetric encryption KMS keys, omit the parameter or specify ENCRYPT_DECRYPT . For HMAC KMS keys
        # (symmetric), specify GENERATE_VERIFY_MAC . For asymmetric KMS keys with RSA key pairs, specify
        # ENCRYPT_DECRYPT or SIGN_VERIFY . For asymmetric KMS keys with NIST-standard elliptic curve key
        # pairs, specify SIGN_VERIFY or KEY_AGREEMENT . For asymmetric KMS keys with ECC_SECG_P256K1 key
        ...
        # You cannot change this value after you create the KMS key. For a multi-Region key, set this
        # parameter to True . For a single-Region KMS key, omit this parameter or set it to False . The
        # default value is False . This operation supports multi-Region keys , an KMS feature that lets you
        # create multiple interoperable KMS keys in different Amazon Web Services Regions. Because these KMS
        # keys have the same key ID, key material, and other metadata, you can use them interchangeably to
        # encrypt data in one Amazon Web Services Region and decrypt it in a different Amazon Web Services
        # Region without re-encrypting the data or making a cross-Region call. For more information about
        # multi-Region keys, see Multi-Region keys in KMS in the Key Management Service Developer Guide . This
        # value creates a primary key , not a replica. To create a replica key , use the ReplicateKey
        ...
        # The source of the key material for the KMS key. You cannot change the origin after you create the
        # KMS key. The default is AWS_KMS , which means that KMS creates the key material. To create a KMS key
        # with no key material (for imported key material), set this value to EXTERNAL . For more information
        # about importing key material into KMS, see Importing Key Material in the Key Management Service
        # Developer Guide . The EXTERNAL origin value is valid only for symmetric KMS keys. To create a KMS
        # key in an CloudHSM key store and create its key material in the associated CloudHSM cluster, set
        # this value to AWS_CLOUDHSM . You must also use the CustomKeyStoreId parameter to identify the
        # CloudHSM key store. The KeySpec value must be SYMMETRIC_DEFAULT . To create a KMS key in an external
        ...
        # The key policy to attach to the KMS key. If you provide a key policy, it must meet the following
        # criteria: The key policy must allow the calling principal to make a subsequent PutKeyPolicy request
        # on the KMS key. This reduces the risk that the KMS key becomes unmanageable. For more information,
        # see Default key policy in the Key Management Service Developer Guide . (To omit this condition, set
        # BypassPolicyLockoutSafetyCheck to true.) Each statement in the key policy must contain one or more
        # principals. The principals in the key policy must exist and be visible to KMS. When you create a new
        # Amazon Web Services principal, you might need to enforce a delay before including the new principal
        # in a key policy because the new principal might not be immediately visible to KMS. For more
        # information, see Changes that I make are not always immediately visible in the Amazon Web Services
        # Identity and Access Management User Guide . If either of the required Resource or Action elements
        # are missing from a key policy statement, the policy statement has no effect. When a key policy
        # statement is missing one of these elements, the KMS console correctly reports an error, but the
        # CreateKey and PutKeyPolicy API requests succeed, even though the policy statement is ineffective.
        # For more information on required key policy elements, see Elements in a key policy in the Key
        # Management Service Developer Guide . If you do not provide a key policy, KMS attaches a default key
        # policy to the KMS key. For more information, see Default key policy in the Key Management Service
        ...
        # To tag an existing KMS key, use the TagResource operation. Do not include confidential or sensitive
        # information in this field. This field may be displayed in plaintext in CloudTrail logs and other
        # output. Tagging or untagging a KMS key can allow or deny permission to the KMS key. For details, see
        # ABAC for KMS in the Key Management Service Developer Guide . To use this parameter, you must have
        # kms:TagResource permission in an IAM policy. Each tag consists of a tag key and a tag value. Both
        # the tag key and the tag value are required, but the tag value can be an empty (null) string. You
        # cannot have more than one tag on a KMS key with the same tag key. If you specify an existing tag key
        # with a different tag value, KMS replaces the current tag value with the specified one. When you add
        ...
        # the documentation for your external key store proxy. This parameter is required for a KMS key with
        # an Origin value of EXTERNAL_KEY_STORE . It is not valid for KMS keys with any other Origin value.
        # The external key must be an existing 256-bit AES symmetric encryption key hosted outside of Amazon
        # Web Services in an external key manager associated with the external key store specified by the
        # CustomKeyStoreId parameter. This key must be enabled and configured to perform encryption and
        # decryption. Each KMS key in an external key store must use a different external key. For details,
        # see Requirements for a KMS key in an external key store in the Key Management Service Developer
        # Guide . Each KMS key in an external key store is associated two backing keys. One is key material
        # that KMS generates. The other is the external key specified by this parameter. When you use the KMS
      ...
      struct CreateKeyResponse
        ...
        def initialize(
          @key_metadata : Types::KeyMetadata? = nil
        )
        end
      end
      ...
      struct CustomKeyStoresListEntry
        ...
        # FAILED . Many failures can be resolved by updating the properties of the custom key store. To update
        # a custom key store, disconnect it ( DisconnectCustomKeyStore ), correct the errors (
        # UpdateCustomKeyStore ), and try to connect again ( ConnectCustomKeyStore ). For additional help
        # resolving these errors, see How to Fix a Connection Failure in Key Management Service Developer
        # Guide . All custom key stores: INTERNAL_ERROR — KMS could not complete the request due to an
        # internal error. Retry the request. For ConnectCustomKeyStore requests, disconnect the custom key
        # store before trying to connect again. NETWORK_ERRORS — Network errors are preventing KMS from
        # connecting the custom key store to its backing key store. CloudHSM key stores: CLUSTER_NOT_FOUND —
        # KMS cannot find the CloudHSM cluster with the specified cluster ID. INSUFFICIENT_CLOUDHSM_HSMS — The
        # associated CloudHSM cluster does not contain any active HSMs. To connect a custom key store to its
        # CloudHSM cluster, the cluster must contain at least one active HSM.
        # INSUFFICIENT_FREE_ADDRESSES_IN_SUBNET — At least one private subnet associated with the CloudHSM
        # cluster doesn't have any available IP addresses. A CloudHSM key store connection requires one free
        # IP address in each of the associated private subnets, although two are preferable. For details, see
        # How to Fix a Connection Failure in the Key Management Service Developer Guide . INVALID_CREDENTIALS
        # — The KeyStorePassword for the custom key store doesn't match the current password of the kmsuser
        # crypto user in the CloudHSM cluster. Before you can connect your custom key store to its CloudHSM
        # cluster, you must change the kmsuser account password and update the KeyStorePassword value for the
        # custom key store. SUBNET_NOT_FOUND — A subnet in the CloudHSM cluster configuration was deleted. If
        # KMS cannot find all of the subnets in the cluster configuration, attempts to connect the custom key
        # store to the CloudHSM cluster fail. To fix this error, create a cluster from a recent backup and
        # associate it with your custom key store. (This process creates a new cluster configuration with a
        # VPC and private subnets.) For details, see How to Fix a Connection Failure in the Key Management
        # Service Developer Guide . USER_LOCKED_OUT — The kmsuser CU account is locked out of the associated
        # CloudHSM cluster due to too many failed password attempts. Before you can connect your custom key
        # store to its CloudHSM cluster, you must change the kmsuser account password and update the key store
        # password value for the custom key store. USER_LOGGED_IN — The kmsuser CU account is logged into the
        # associated CloudHSM cluster. This prevents KMS from rotating the kmsuser account password and
        # logging into the cluster. Before you can connect your custom key store to its CloudHSM cluster, you
        # must log the kmsuser CU out of the cluster. If you changed the kmsuser password to log into the
        # cluster, you must also and update the key store password value for the custom key store. For help,
        # see How to Log Out and Reconnect in the Key Management Service Developer Guide . USER_NOT_FOUND —
        # KMS cannot find a kmsuser CU account in the associated CloudHSM cluster. Before you can connect your
        # custom key store to its CloudHSM cluster, you must create a kmsuser CU account in the cluster, and
        # then update the key store password value for the custom key store. External key stores:
        # INVALID_CREDENTIALS — One or both of the XksProxyAuthenticationCredential values is not valid on the
        # specified external key store proxy. XKS_PROXY_ACCESS_DENIED — KMS requests are denied access to the
        # external key store proxy. If the external key store proxy has authorization rules, verify that they
        # permit KMS to communicate with the proxy on your behalf. XKS_PROXY_INVALID_CONFIGURATION — A
        # configuration error is preventing the external key store from connecting to its proxy. Verify the
        # value of the XksProxyUriPath . XKS_PROXY_INVALID_RESPONSE — KMS cannot interpret the response from
        # the external key store proxy. If you see this connection error code repeatedly, notify your external
        # key store proxy vendor. XKS_PROXY_INVALID_TLS_CONFIGURATION — KMS cannot connect to the external key
        # store proxy because the TLS configuration is invalid. Verify that the XKS proxy supports TLS 1.2 or
        # 1.3. Also, verify that the TLS certificate is not expired, and that it matches the hostname in the
        # XksProxyUriEndpoint value, and that it is signed by a certificate authority included in the Trusted
        # Certificate Authorities list. XKS_PROXY_NOT_REACHABLE — KMS can't communicate with your external key
        # store proxy. Verify that the XksProxyUriEndpoint and XksProxyUriPath are correct. Use the tools for
        # your external key store proxy to verify that the proxy is active and available on its network. Also,
        # verify that your external key manager instances are operating properly. Connection attempts fail
        # with this connection error code if the proxy reports that all external key manager instances are
        # unavailable. XKS_PROXY_TIMED_OUT — KMS can connect to the external key store proxy, but the proxy
        # does not respond to KMS in the time allotted. If you see this connection error code repeatedly,
        # notify your external key store proxy vendor. XKS_VPC_ENDPOINT_SERVICE_INVALID_CONFIGURATION — The
        # Amazon VPC endpoint service configuration doesn't conform to the requirements for an KMS external
        # key store. The VPC endpoint service must be an endpoint service for interface endpoints in the
        # caller's Amazon Web Services account. It must have a network load balancer (NLB) connected to at
        # least two subnets, each in a different Availability Zone. The Allow principals list must include the
        # KMS service principal for the Region, cks.kms.&lt;region&gt;.amazonaws.com , such as
        # cks.kms.us-east-1.amazonaws.com . It must not require acceptance of connection requests. It must
        # have a private DNS name. The private DNS name for an external key store with VPC_ENDPOINT_SERVICE
        # connectivity must be unique in its Amazon Web Services Region. The domain of the private DNS name
        # must have a verification status of verified . The TLS certificate specifies the private DNS hostname
        # at which the endpoint is reachable. XKS_VPC_ENDPOINT_SERVICE_NOT_FOUND — KMS can't find the VPC
        ...
        # store, the ConnectionState indicates whether it is connected to its CloudHSM cluster. For an
        # external key store, the ConnectionState indicates whether it is connected to the external key store
        # proxy that communicates with your external key manager. You can create and use KMS keys in your
        # custom key stores only when its ConnectionState is CONNECTED . The ConnectionState value is
        # DISCONNECTED only if the key store has never been connected or you use the DisconnectCustomKeyStore
        # operation to disconnect it. If the value is CONNECTED but you are having trouble using the custom
        # key store, make sure that the backing key store is reachable and active. For an CloudHSM key store,
        # verify that its associated CloudHSM cluster is active and contains at least one active HSM. For an
        # external key store, verify that the external key store proxy and external key manager are connected
        # and enabled. A value of FAILED indicates that an attempt to connect was unsuccessful. The
        ...
        def initialize(
          @cloud_hsm_cluster_id : String? = nil,
          @connection_error_code : String? = nil,
          ...
          @custom_key_store_type : String? = nil,
          @trust_anchor_certificate : String? = nil,
          @xks_proxy_configuration : Types::XksProxyConfigurationType? = nil
        )
        end
      end
      ...
      struct DecryptRequest
        ...
        @[JSON::Field(key: "CiphertextBlob", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        # only for cryptographic operations with a symmetric encryption KMS key. The standard asymmetric
        # encryption algorithms and HMAC algorithms that KMS uses do not support an encryption context. An
        # encryption context is a collection of non-secret key-value pairs that represent additional
        # authenticated data. When you use an encryption context to encrypt data, you must specify the same
        # (an exact case-sensitive match) encryption context to decrypt the data. An encryption context is
        ...
        # Specifies the KMS key that KMS uses to decrypt the ciphertext. Enter a key ID of the KMS key that
        # was used to encrypt the ciphertext. If you identify a different KMS key, the Decrypt operation
        # throws an IncorrectKeyException . This parameter is required only when the ciphertext was encrypted
        # under an asymmetric KMS key. If you used a symmetric encryption KMS key, KMS can get the KMS key
        # from metadata that it adds to the symmetric ciphertext blob. However, it is always recommended as a
        # best practice. This practice ensures that you use the KMS key that you intend. To specify a KMS key,
        # use its key ID, key ARN, alias name, or alias ARN. When using an alias name, prefix it with "alias/"
        # . To specify a KMS key in a different Amazon Web Services account, you must use the key ARN or alias
        # ARN. For example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        ...
        # encryption algorithm to use with the public key in the attestation document. The only valid
        # encryption algorithm is RSAES_OAEP_SHA_256 . This parameter supports the Amazon Web Services Nitro
        # Enclaves SDK or any Amazon Web Services SDK for Amazon Web Services Nitro Enclaves. It supports any
        # Amazon Web Services SDK for Amazon Web Services NitroTPM. When you use this parameter, instead of
        # returning the plaintext data, KMS encrypts the plaintext data with the public key in the attestation
        # document, and returns the resulting ciphertext in the CiphertextForRecipient field in the response.
        # This ciphertext can be decrypted only with the private key in the attested environment. The
      ...
      struct DecryptResponse
        ...
        # The plaintext data encrypted with the public key from the attestation document. This ciphertext can
        # be decrypted only by using a private key from the attested environment. This field is included in
        # the response only when the Recipient parameter in the request includes a valid attestation document
        ...
        # attestation support in KMS in the Key Management Service Developer Guide .
        @[JSON::Field(key: "CiphertextForRecipient", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        @[JSON::Field(key: "Plaintext", converter: Aws::Runtime::Base64BytesConverter)]
      ...
      struct DeleteImportedKeyMaterialRequest
        ...
        # Identifies the KMS key from which you are deleting imported key material. The Origin of the KMS key
        # must be EXTERNAL . Specify the key ID or key ARN of the KMS key. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct DeriveSharedSecretRequest
        ...
        # Identifies an asymmetric NIST-standard ECC or SM2 (China Regions only) KMS key. KMS uses the private
        # key in the specified key pair to derive the shared secret. The key usage of the KMS key must be
        # KEY_AGREEMENT . To find the KeyUsage of a KMS key, use the DescribeKey operation. To specify a KMS
        # key, use its key ID, key ARN, alias name, or alias ARN. When using an alias name, prefix it with
        ...
        # only) key pair. The public key must be a DER-encoded X.509 public key, also known as
        # SubjectPublicKeyInfo (SPKI), as defined in RFC 5280 . GetPublicKey returns the public key of an
        # asymmetric KMS key pair in the required DER-encoded format. If you use Amazon Web Services CLI
        # version 1 , you must provide the DER-encoded X.509 public key in a file. Otherwise, the Amazon Web
        ...
        @[JSON::Field(key: "PublicKey", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        # A signed attestation document from an Amazon Web Services Nitro enclave or NitroTPM, and the
        # encryption algorithm to use with the public key in the attestation document. The only valid
        # encryption algorithm is RSAES_OAEP_SHA_256 . This parameter only supports attestation documents for
        # Amazon Web Services Nitro Enclaves or Amazon Web Services NitroTPM. To call DeriveSharedSecret
        # generate an attestation document use either Amazon Web Services Nitro Enclaves SDK for an Amazon Web
        # Services Nitro Enclaves or Amazon Web Services NitroTPM tools for Amazon Web Services NitroTPM. Then
        # use the Recipient parameter from any Amazon Web Services SDK to provide the attestation document for
        # the attested environment. When you use this parameter, instead of returning a plaintext copy of the
        # shared secret, KMS encrypts the plaintext shared secret under the public key in the attestation
        # document, and returns the resulting ciphertext in the CiphertextForRecipient field in the response.
        # This ciphertext can be decrypted only with the private key in the attested environment. The
        # CiphertextBlob field in the response contains the encrypted shared secret derived from the KMS key
        # specified by the KeyId parameter and public key specified by the PublicKey parameter. The
      ...
      struct DeriveSharedSecretResponse
        ...
        # ciphertext can be decrypted only by using a private key from the attested environment. This field is
        # included in the response only when the Recipient parameter in the request includes a valid
        ...
        # The source of the key material for the specified KMS key. When this value is AWS_KMS , KMS created
        # the key material. When this value is EXTERNAL , the key material was imported or the KMS key doesn't
        ...
        @[JSON::Field(key: "SharedSecret", converter: Aws::Runtime::Base64BytesConverter)]
        getter shared_secret : Bytes?

        def initialize(
          @ciphertext_for_recipient : Bytes? = nil,
          @key_agreement_algorithm : String? = nil,
          @key_id : String? = nil,
          @key_origin : String? = nil,
      ...
      struct DescribeCustomKeyStoresRequest
        ...
        # Gets only information about the specified custom key store. Enter the key store ID. By default, this
      ...
      struct DescribeCustomKeyStoresResponse
        ...
        def initialize(
          @custom_key_stores : Array(Types::CustomKeyStoresListEntry)? = nil,
      ...
      struct DescribeKeyRequest
        ...
        # Describes the specified KMS key. If you specify a predefined Amazon Web Services alias (an Amazon
        # Web Services alias with no key ID), KMS associates the alias with an Amazon Web Services managed key
        # and returns its KeyId and Arn in the response. To specify a KMS key, use its key ID, key ARN, alias
        # name, or alias ARN. When using an alias name, prefix it with "alias/" . To specify a KMS key in a
        # different Amazon Web Services account, you must use the key ARN or alias ARN. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab Alias name:
        # alias/ExampleAlias Alias ARN: arn:aws:kms:us-east-2:111122223333:alias/ExampleAlias To get the key
        # ID and key ARN for a KMS key, use ListKeys or DescribeKey . To get the alias name and alias ARN, use
      ...
      struct DisableKeyRequest
        ...
        # Identifies the KMS key to disable. Specify the key ID or key ARN of the KMS key. For example: Key
      ...
      struct DisableKeyRotationRequest
        ...
        # asymmetric KMS keys , HMAC KMS keys , KMS keys with imported key material , or KMS keys in a custom
        # key store . Specify the key ID or key ARN of the KMS key. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct EnableKeyRotationRequest
        ...
        # keys , HMAC KMS keys , KMS keys with imported key material , or KMS keys in a custom key store . To
        # enable or disable automatic rotation of a set of related multi-Region keys , set the property on the
        # primary key. Specify the key ID or key ARN of the KMS key. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
        ...
        # specified, the default value is 365 days. The rotation period defines the number of days after you
      ...
      struct EncryptRequest
        ...
        # ENCRYPT_DECRYPT . To find the KeyUsage of a KMS key, use the DescribeKey operation. To specify a KMS
        # key, use its key ID, key ARN, alias name, or alias ARN. When using an alias name, prefix it with
        # "alias/" . To specify a KMS key in a different Amazon Web Services account, you must use the key ARN
        # or alias ARN. For example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab Alias name:
        ...
        # must be compatible with the KMS key that you specify. This parameter is required only for asymmetric
        ...
        # valid only for cryptographic operations with a symmetric encryption KMS key. The standard asymmetric
        # encryption algorithms and HMAC algorithms that KMS uses do not support an encryption context. Do not
        # include confidential or sensitive information in this field. This field may be displayed in
        # plaintext in CloudTrail logs and other output. An encryption context is a collection of non-secret
        # key-value pairs that represent additional authenticated data. When you use an encryption context to
        # encrypt data, you must specify the same (an exact case-sensitive match) encryption context to
        # decrypt the data. An encryption context is supported only on operations with symmetric encryption
      ...
      struct GenerateDataKeyPairRequest
        ...
        # cannot specify an asymmetric KMS key or a KMS key in a custom key store. To get the type and origin
        # of your KMS key, use the DescribeKey operation. To specify a KMS key, use its key ID, key ARN, alias
        # name, or alias ARN. When using an alias name, prefix it with "alias/" . To specify a KMS key in a
        # different Amazon Web Services account, you must use the key ARN or alias ARN. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab Alias name:
        ...
        # Determines the type of data key pair that is generated. The KMS rule that restricts the use of
        # asymmetric RSA and SM2 KMS keys to encrypt and decrypt or to sign and verify (but not both), the
        ...
        # pair. Do not include confidential or sensitive information in this field. This field may be
        # displayed in plaintext in CloudTrail logs and other output. An encryption context is a collection of
        # non-secret key-value pairs that represent additional authenticated data. When you use an encryption
        # context to encrypt data, you must specify the same (an exact case-sensitive match) encryption
        # context to decrypt the data. An encryption context is supported only on operations with symmetric
        ...
        # encryption algorithm to use with the public key in the attestation document. The only valid
        # encryption algorithm is RSAES_OAEP_SHA_256 . This parameter only supports attestation documents for
        # Amazon Web Services Nitro Enclaves or Amazon Web Services NitroTPM. To call GenerateDataKeyPair
        # generate an attestation document use either Amazon Web Services Nitro Enclaves SDK for an Amazon Web
        # Services Nitro Enclaves or Amazon Web Services NitroTPM tools for Amazon Web Services NitroTPM. Then
        # use the Recipient parameter from any Amazon Web Services SDK to provide the attestation document for
        # the attested environment. When you use this parameter, instead of returning a plaintext copy of the
        # private data key, KMS encrypts the plaintext private data key under the public key in the
        # attestation document, and returns the resulting ciphertext in the CiphertextForRecipient field in
        # the response. This ciphertext can be decrypted only with the private key in the attested
        # environment. The CiphertextBlob field in the response contains a copy of the private data key
        # encrypted under the KMS key specified by the KeyId parameter. The PrivateKeyPlaintext field in the
      ...
      struct GenerateDataKeyPairResponse
        ...
        @[JSON::Field(key: "PrivateKeyCiphertextBlob", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        @[JSON::Field(key: "PrivateKeyPlaintext", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        def initialize(
          ...
          @key_id : String? = nil,
          @key_material_id : String? = nil,
          @key_pair_spec : String? = nil,
          @private_key_ciphertext_blob : Bytes? = nil,
          @private_key_plaintext : Bytes? = nil,
      ...
      struct GenerateDataKeyRequest
        ...
        # use the DescribeKey operation. To specify a KMS key, use its key ID, key ARN, alias name, or alias
        # ARN. When using an alias name, prefix it with "alias/" . To specify a KMS key in a different Amazon
        ...
        # Specifies the encryption context that will be used when encrypting the data key. Do not include
        # confidential or sensitive information in this field. This field may be displayed in plaintext in
        # CloudTrail logs and other output. An encryption context is a collection of non-secret key-value
        # pairs that represent additional authenticated data. When you use an encryption context to encrypt
        # data, you must specify the same (an exact case-sensitive match) encryption context to decrypt the
        # data. An encryption context is supported only on operations with symmetric encryption KMS keys. On
        ...
        # Enclaves SDK or any Amazon Web Services SDK for Amazon Web Services Nitro Enclaves. It supports any
        # Amazon Web Services SDK for Amazon Web Services NitroTPM. When you use this parameter, instead of
        # returning the plaintext data key, KMS encrypts the plaintext data key under the public key in the
        # attestation document, and returns the resulting ciphertext in the CiphertextForRecipient field in
        # the response. This ciphertext can be decrypted only with the private key in the enclave. The
        # CiphertextBlob field in the response contains a copy of the data key encrypted under the KMS key
        # specified by the KeyId parameter. The Plaintext field in the response is null or empty. For
      ...
      struct GenerateDataKeyResponse
        ...
        # The plaintext data key encrypted with the public key from the attestation document. This ciphertext
        # can be decrypted only by using a private key from the attested environment. This field is included
        # in the response only when the Recipient parameter in the request includes a valid attestation
      ...
      struct GenerateMacRequest
        ...
        @[JSON::Field(key: "Message", converter: Aws::Runtime::Base64BytesConverter)]
      ...
      struct GenerateMacResponse
        ...
        @[JSON::Field(key: "Mac", converter: Aws::Runtime::Base64BytesConverter)]
        getter mac : Bytes?
      ...
      struct GenerateRandomRequest
        ...
        # Enclaves SDK or any Amazon Web Services SDK for Amazon Web Services Nitro Enclaves. It supports any
        # Amazon Web Services SDK for Amazon Web Services NitroTPM. When you use this parameter, instead of
        # returning plaintext bytes, KMS encrypts the plaintext bytes under the public key in the attestation
        # document, and returns the resulting ciphertext in the CiphertextForRecipient field in the response.
        # This ciphertext can be decrypted only with the private key in the attested environment. The
        # Plaintext field in the response is null or empty. For information about the interaction between KMS
        # and Amazon Web Services Nitro Enclaves or Amazon Web Services NitroTPM, see Cryptographic
      ...
      struct GetKeyPolicyRequest
        ...
        # Gets the key policy for the specified KMS key. Specify the key ID or key ARN of the KMS key. For
      ...
      struct GetKeyRotationStatusRequest
        ...
        # Gets the rotation status for the specified KMS key. Specify the key ID or key ARN of the KMS key. To
        # specify a KMS key in a different Amazon Web Services account, you must use the key ARN. For example:
      ...
      struct GetKeyRotationStatusResponse
        ...
        # Identifies the date and time that an in progress on-demand rotation was initiated. KMS uses a
        # background process to perform rotations. As a result, there might be a slight delay between
        ...
        @[JSON::Field(key: "OnDemandRotationStartDate", converter: Aws::Runtime::UnixTimestampConverter)]
      ...
      struct GetParametersForImportRequest
        ...
        # The identifier of the KMS key that will be associated with the imported key material. The Origin of
        # the KMS key must be EXTERNAL . All KMS key types are supported, including multi-Region keys.
        # However, you cannot import key material into a KMS key in a custom key store. Specify the key ID or
        ...
        # The algorithm you will use with the RSA public key ( PublicKey ) in the response to protect your key
        # material during import. For more information, see Select a wrapping algorithm in the Key Management
        # Service Developer Guide . For RSA_AES wrapping algorithms, you encrypt your key material with an AES
        # key that you generate, then encrypt your AES key with the RSA public key from KMS. For RSAES
        # wrapping algorithms, you encrypt your key material directly with the RSA public key from KMS. The
        # wrapping algorithms that you can use depend on the type of key material that you are importing. To
        # import an RSA private key, you must use an RSA_AES wrapping algorithm. RSA_AES_KEY_WRAP_SHA_256 —
        # Supported for wrapping RSA and ECC key material. RSA_AES_KEY_WRAP_SHA_1 — Supported for wrapping RSA
        # and ECC key material. RSAES_OAEP_SHA_256 — Supported for all types of key material, except RSA key
        # material (private key). You cannot use the RSAES_OAEP_SHA_256 wrapping algorithm with the RSA_2048
        # wrapping key spec to wrap ECC_NIST_P521 key material. RSAES_OAEP_SHA_1 — Supported for all types of
        ...
        # specified wrapping algorithm to protect your key material during import. Use the longest RSA
      ...
      struct GetParametersForImportResponse
        ...
        @[JSON::Field(key: "ImportToken", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        @[JSON::Field(key: "ParametersValidTo", converter: Aws::Runtime::UnixTimestampConverter)]
      ...
      struct GetPublicKeyRequest
        ...
        # Identifies the asymmetric KMS key that includes the public key. To specify a KMS key, use its key
        # ID, key ARN, alias name, or alias ARN. When using an alias name, prefix it with "alias/" . To
      ...
      struct GetPublicKeyResponse
        ...
        # The encryption algorithms that KMS supports for this key. This information is critical. If a public
        ...
        def initialize(
          @customer_master_key_spec : String? = nil,
          @encryption_algorithms : Array(String)? = nil,
          @key_agreement_algorithms : Array(String)? = nil,
          @key_id : String? = nil,
          @key_spec : String? = nil,
          @key_usage : String? = nil,
          @public_key : Bytes? = nil,
          @signing_algorithms : Array(String)? = nil
        )
        end
      end

      # Use this structure to allow cryptographic operations in the grant only when the operation request
      # includes the specified encryption context . KMS applies the grant constraints only to cryptographic
      # operations that support an encryption context, that is, all cryptographic operations with a
      # symmetric KMS key. Grant constraints are not applied to operations that do not support an encryption
      # context, such as cryptographic operations with asymmetric KMS keys and management operations, such
      # as DescribeKey or RetireGrant . In a cryptographic operation, the encryption context in the
      # decryption operation must be an exact, case-sensitive match for the keys and values in the
      # encryption context of the encryption operation. Only the order of the pairs can vary. However, in a
      # grant constraint, the key in each key-value pair is not case sensitive, but the value is case
      # sensitive. To avoid confusion, do not use multiple encryption context pairs that differ only by
      ...
      struct GrantListEntry
        ...
        # The identity that gets the permissions in the grant. The GranteePrincipal field in the ListGrants
        # response usually contains the user or role designated as the grantee principal in the grant.
      ...
      struct ImportKeyMaterialRequest
        ...
        @[JSON::Field(key: "EncryptedKeyMaterial", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        # The identifier of the KMS key that will be associated with the imported key material. This must be
        # the same KMS key specified in the KeyID parameter of the corresponding GetParametersForImport
        # request. The Origin of the KMS key must be EXTERNAL and its KeyState must be PendingImport . The KMS
        # key can be a symmetric encryption KMS key, HMAC KMS key, asymmetric encryption KMS key, or
        # asymmetric signing KMS key, including a multi-Region key of any supported type. You cannot perform
        # this operation on a KMS key in a custom key store, or on a KMS key in a different Amazon Web
        # Services account. Specify the key ID or key ARN of the KMS key. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
        ...
        # choice, see Setting an expiration time in the Key Management Service Developer Guide . When the
        # value of ExpirationModel is KEY_MATERIAL_EXPIRES , you must specify a value for the ValidTo
        ...
        # This parameter is optional and only usable with symmetric encryption keys. If no key material has
        # ever been imported into the KMS key, and this parameter is omitted, the parameter defaults to
        # NEW_KEY_MATERIAL . After the first key material is imported, if this parameter is omitted then the
        # parameter defaults to EXISTING_KEY_MATERIAL . For multi-Region keys, you must first import new key
        ...
        # NEW_KEY_MATERIAL . Whenever you import key material into a symmetric encryption key, KMS assigns a
        # unique identifier to the key material based on the KMS key ID and the imported key material. When
        # you re-import key material with a specified key material ID, KMS: Computes the identifier for the
        ...
        # The date and time when the imported key material expires. This parameter is required when the value
        # of the ExpirationModel parameter is KEY_MATERIAL_EXPIRES . Otherwise it is not valid. The value of
        # this parameter must be a future date and time. The maximum value is 365 days from the request date.
        ...
        @[JSON::Field(key: "ValidTo", converter: Aws::Runtime::UnixTimestampConverter)]
      ...
      struct KeyMetadata
        ...
        # The date and time after which KMS deletes this KMS key. This value is present only when the KMS key
        # is scheduled for deletion, that is, when its KeyState is PendingDeletion . When the primary key in a
        ...
        # value of the MultiRegion field is True . For more information about any listed KMS key, use the
        # DescribeKey operation. MultiRegionKeyType indicates whether the KMS key is a PRIMARY or REPLICA key.
        ...
        # begins when the last of its replica keys is deleted. This value is present only when the KeyState of
        # the KMS key is PendingReplicaDeletion . That indicates that the KMS key is the primary key in a
        # multi-Region key, it is scheduled for deletion, and it still has existing replica keys. When a
        # single-Region KMS key or a multi-Region replica key is scheduled for deletion, its deletion date is
        # displayed in the DeletionDate field. However, when the primary key in a multi-Region key is
        # scheduled for deletion, its waiting period doesn't begin until all of its replica keys are deleted.
        ...
        def initialize(
          ...
          @expiration_model : String? = nil,
          @key_agreement_algorithms : Array(String)? = nil,
          @key_manager : String? = nil,
          @key_spec : String? = nil,
          @key_state : String? = nil,
          @key_usage : String? = nil,
          @mac_algorithms : Array(String)? = nil,
          @multi_region : Bool? = nil,
          @multi_region_configuration : Types::MultiRegionConfiguration? = nil,
          @origin : String? = nil,
          @pending_deletion_window_in_days : Int32? = nil,
          @signing_algorithms : Array(String)? = nil,
          @valid_to : Time? = nil,
          @xks_key_configuration : Types::XksKeyConfigurationType? = nil
        )
        end
      end
      ...
      struct ListAliasesRequest
        ...
        # Lists only aliases that are associated with the specified KMS key. Enter a KMS key in your Amazon
        # Web Services account. This parameter is optional. If you omit it, ListAliases returns all aliases in
        # the account and Region. Specify the key ID or key ARN of the KMS key. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct ListAliasesResponse
        ...
        def initialize(
          @aliases : Array(Types::AliasListEntry)? = nil,
          @next_marker : String? = nil,
          @truncated : Bool? = nil
        )
        end
      ...
      struct ListGrantsRequest
        ...
        # Returns only grants for the specified KMS key. This parameter is required. Specify the key ID or key
        # ARN of the KMS key. To specify a KMS key in a different Amazon Web Services account, you must use
      ...
      struct ListGrantsResponse
        ...
        def initialize(
          @grants : Array(Types::GrantListEntry)? = nil,
          @next_marker : String? = nil,
          @truncated : Bool? = nil
        )
        end
      ...
      struct ListKeyPoliciesRequest
        ...
        # Gets the names of key policies for the specified KMS key. Specify the key ID or key ARN of the KMS
        # key. For example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct ListKeyRotationsRequest
        ...
        # Gets the key rotations for the specified KMS key. Specify the key ID or key ARN of the KMS key. For
        # example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
        ...
        # Use this optional parameter to control which key materials associated with this key are listed in
        # the response. The default value of this parameter is ROTATIONS_ONLY . If you omit this parameter,
        # KMS returns information on the key materials created by automatic or on-demand key rotation. When
      ...
      struct ListKeyRotationsResponse
        ...
        def initialize(
          @next_marker : String? = nil,
          @rotations : Array(Types::RotationsListEntry)? = nil,
          @truncated : Bool? = nil
        )
        end
      end
      ...
      struct ListKeysResponse
        ...
        def initialize(
          @keys : Array(Types::KeyListEntry)? = nil,
          @next_marker : String? = nil,
          @truncated : Bool? = nil
        )
        end
      ...
      struct ListResourceTagsRequest
        ...
        # Gets tags on the specified KMS key. Specify the key ID or key ARN of the KMS key. For example: Key
        # ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct ListRetirableGrantsRequest
        ...
        # account. To specify the retiring principal, use the Amazon Resource Name (ARN) of an Amazon Web
      ...
      struct MultiRegionConfiguration
        ...
        def initialize(
          ...
          @primary_key : Types::MultiRegionKey? = nil,
          @replica_keys : Array(Types::MultiRegionKey)? = nil
        )
        end
      end
      ...
      struct PutKeyPolicyRequest
        ...
        # Sets the key policy on the specified KMS key. Specify the key ID or key ARN of the KMS key. For
        # example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
        ...
        # The key policy to attach to the KMS key. The key policy must meet the following criteria: The key
        # policy must allow the calling principal to make a subsequent PutKeyPolicy request on the KMS key.
        # This reduces the risk that the KMS key becomes unmanageable. For more information, see Default key
        # policy in the Key Management Service Developer Guide . (To omit this condition, set
        # BypassPolicyLockoutSafetyCheck to true.) Each statement in the key policy must contain one or more
        # principals. The principals in the key policy must exist and be visible to KMS. When you create a new
        # Amazon Web Services principal, you might need to enforce a delay before including the new principal
        # in a key policy because the new principal might not be immediately visible to KMS. For more
        ...
        # are missing from a key policy statement, the policy statement has no effect. When a key policy
        # statement is missing one of these elements, the KMS console correctly reports an error, but the
        # PutKeyPolicy API request succeeds, even though the policy statement is ineffective. For more
        # information on required key policy elements, see Elements in a key policy in the Key Management
        # Service Developer Guide . A key policy document can include only the following characters: Printable
        # ASCII characters from the space character ( \u0020 ) through the end of the ASCII character range.
        # Printable characters in the Basic Latin and Latin-1 Supplement character set (through \u00FF ). The
        # tab ( \u0009 ), line feed ( \u000A ), and carriage return ( \u000D ) special characters If the key
        # policy exceeds the length constraint, KMS returns a LimitExceededException . For information about
      ...
      struct ReEncryptRequest
        ...
        # encryption KMS key or an asymmetric KMS key with a KeyUsage value of ENCRYPT_DECRYPT . To find the
        # KeyUsage value of a KMS key, use the DescribeKey operation. To specify a KMS key, use its key ID,
        # key ARN, alias name, or alias ARN. When using an alias name, prefix it with "alias/" . To specify a
        # KMS key in a different Amazon Web Services account, you must use the key ARN or alias ARN. For
        # example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab Alias name:
        # alias/ExampleAlias Alias ARN: arn:aws:kms:us-east-2:111122223333:alias/ExampleAlias To get the key
        # ID and key ARN for a KMS key, use ListKeys or DescribeKey . To get the alias name and alias ARN, use
        ...
        # Specifies that encryption context to use when the reencrypting the data. Do not include confidential
        # or sensitive information in this field. This field may be displayed in plaintext in CloudTrail logs
        # and other output. A destination encryption context is valid only when the destination KMS key is a
        # symmetric encryption KMS key. The standard ciphertext format for asymmetric KMS keys does not
        # include fields for metadata. An encryption context is a collection of non-secret key-value pairs
        # that represent additional authenticated data. When you use an encryption context to encrypt data,
        # you must specify the same (an exact case-sensitive match) encryption context to decrypt the data. An
        # encryption context is supported only on operations with symmetric encryption KMS keys. On operations
        ...
        # that was used to encrypt the ciphertext. An encryption context is a collection of non-secret
        # key-value pairs that represent additional authenticated data. When you use an encryption context to
        # encrypt data, you must specify the same (an exact case-sensitive match) encryption context to
        # decrypt the data. An encryption context is supported only on operations with symmetric encryption
        # KMS keys. On operations with symmetric encryption KMS keys, an encryption context is optional, but
        ...
        # Specifies the KMS key that KMS will use to decrypt the ciphertext before it is re-encrypted. Enter a
        # key ID of the KMS key that was used to encrypt the ciphertext. If you identify a different KMS key,
        # the ReEncrypt operation throws an IncorrectKeyException . This parameter is required only when the
        # ciphertext was encrypted under an asymmetric KMS key. If you used a symmetric encryption KMS key,
        # KMS can get the KMS key from metadata that it adds to the symmetric ciphertext blob. However, it is
        # always recommended as a best practice. This practice ensures that you use the KMS key that you
        # intend. To specify a KMS key, use its key ID, key ARN, alias name, or alias ARN. When using an alias
        # name, prefix it with "alias/" . To specify a KMS key in a different Amazon Web Services account, you
        # must use the key ARN or alias ARN. For example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key
        # ARN: arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab Alias name:
        # alias/ExampleAlias Alias ARN: arn:aws:kms:us-east-2:111122223333:alias/ExampleAlias To get the key
        # ID and key ARN for a KMS key, use ListKeys or DescribeKey . To get the alias name and alias ARN, use
        # ListAliases .
      ...
      # Contains information about the party that receives the response from the API operation. This data
      # type is designed to support Amazon Web Services Nitro Enclaves and Amazon Web Services NitroTPM,
      ...
      struct RecipientInfo
        ...
        @[JSON::Field(key: "AttestationDocument", converter: Aws::Runtime::Base64BytesConverter)]
        ...
        def initialize(
          ...
          @key_encryption_algorithm : String? = nil
        )
        end
      end
      ...
      struct ReplicateKeyRequest
        ...
        # Identifies the multi-Region primary key that is being replicated. To determine whether a KMS key is
        # a multi-Region primary key, use the DescribeKey operation to check the value of the
        # MultiRegionKeyType property. Specify the key ID or key ARN of a multi-Region primary key. For
        ...
        # The Region ID of the Amazon Web Services Region for this replica key. Enter the Region ID, such as
        # us-east-1 or ap-southeast-2 . For a list of Amazon Web Services Regions in which KMS is supported,
        # see KMS service endpoints in the Amazon Web Services General Reference . The replica must be in a
        # different Amazon Web Services Region than its primary key and other replicas of that primary key,
        # but in the same Amazon Web Services partition. KMS must be available in the replica Region. If the
        # Region is not enabled by default, the Amazon Web Services account must be enabled in the Region. For
        ...
        # A description of the KMS key. The default value is an empty string (no description). Do not include
        # confidential or sensitive information in this field. This field may be displayed in plaintext in
        ...
        # policy, KMS attaches the default key policy to the KMS key. The key policy is not a shared property
        # of multi-Region keys. You can specify the same key policy or a different key policy for each key in
        # a set of related multi-Region keys. KMS does not synchronize this property. If you provide a key
        # policy, it must meet the following criteria: The key policy must allow the calling principal to make
        # a subsequent PutKeyPolicy request on the KMS key. This reduces the risk that the KMS key becomes
        # unmanageable. For more information, see Default key policy in the Key Management Service Developer
        # Guide . (To omit this condition, set BypassPolicyLockoutSafetyCheck to true.) Each statement in the
        # key policy must contain one or more principals. The principals in the key policy must exist and be
        # visible to KMS. When you create a new Amazon Web Services principal, you might need to enforce a
        # delay before including the new principal in a key policy because the new principal might not be
        # immediately visible to KMS. For more information, see Changes that I make are not always immediately
        # visible in the Amazon Web Services Identity and Access Management User Guide . A key policy document
        # can include only the following characters: Printable ASCII characters from the space character (
        # \u0020 ) through the end of the ASCII character range. Printable characters in the Basic Latin and
        # Latin-1 Supplement character set (through \u00FF ). The tab ( \u0009 ), line feed ( \u000A ), and
        ...
        # created. To tag an existing KMS key, use the TagResource operation. Do not include confidential or
        # sensitive information in this field. This field may be displayed in plaintext in CloudTrail logs and
        # other output. Tagging or untagging a KMS key can allow or deny permission to the KMS key. For
        # details, see ABAC for KMS in the Key Management Service Developer Guide . To use this parameter, you
        # must have kms:TagResource permission in an IAM policy. Tags are not a shared property of
        # multi-Region keys. You can specify the same tags or different tags for each key in a set of related
        # multi-Region keys. KMS does not synchronize this property. Each tag consists of a tag key and a tag
        # value. Both the tag key and the tag value are required, but the tag value can be an empty (null)
        # string. You cannot have more than one tag on a KMS key with the same tag key. If you specify an
        # existing tag key with a different tag value, KMS replaces the current tag value with the specified
      ...
      struct ReplicateKeyResponse
        ...
        def initialize(
          @replica_key_metadata : Types::KeyMetadata? = nil,
          @replica_policy : String? = nil,
          @replica_tags : Array(Types::Tag)? = nil
        )
        end
      end
      ...
      struct RevokeGrantRequest
        ...
        # A unique identifier for the KMS key associated with the grant. To get the key ID and key ARN for a
        # KMS key, use ListKeys or DescribeKey . Specify the key ID or key ARN of the KMS key. To specify a
        # KMS key in a different Amazon Web Services account, you must use the key ARN. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct RotateKeyOnDemandRequest
        ...
        # keys , HMAC KMS keys , multi-Region KMS keys with imported key material , or KMS keys in a custom
        # key store . To perform on-demand rotation of a set of related multi-Region keys , invoke the
        # on-demand rotation on the primary key. Specify the key ID or key ARN of the KMS key. For example:
        # Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct RotationsListEntry
        ...
        # Indicates if the key material is configured to automatically expire. There are two possible values
        ...
        # PENDING_MULTI_REGION_IMPORT_AND_ROTATION and PENDING_ROTATION . KMS uses CURRENT key material for
        # both encryption and decryption and NON_CURRENT key material only for decryption. PENDING_ROTATION
        # identifies key material that has been imported for on-demand key rotation but the rotation hasn't
        # completed. The key material state PENDING_MULTI_REGION_IMPORT_AND_ROTATION is unique to
        # multi-region, symmetric encryption keys with imported key material. It indicates key material that
        # has been imported into the primary Region key but not all of the replica Region keys. When this key
        # material is imported in to all of the replica Region keys, the key material state will change to
        # PENDING_ROTATION . Key material in PENDING_MULTI_REGION_IMPORT_AND_ROTATION or PENDING_ROTATION
        # state is not permanently associated with the KMS key. You can delete this key material and import
        # different key material in its place. The PENDING_MULTI_REGION_IMPORT_AND_ROTATION and
        ...
        @[JSON::Field(key: "RotationDate", converter: Aws::Runtime::UnixTimestampConverter)]
        ...
        def initialize(
          @expiration_model : String? = nil,
          @import_state : String? = nil,
          @key_id : String? = nil,
          @key_material_description : String? = nil,
          @key_material_id : String? = nil,
          @key_material_state : String? = nil,
          @rotation_date : Time? = nil,
          @rotation_type : String? = nil,
          @valid_to : Time? = nil
        )
        end
      end
      ...
      struct ScheduleKeyDeletionRequest
        ...
        # The unique identifier of the KMS key to delete. Specify the key ID or key ARN of the KMS key. For
        # example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
        ...
        # key. If the KMS key is a multi-Region primary key with replica keys, the waiting period begins when
        # the last of its replica keys is deleted. Otherwise, the waiting period begins immediately. This
        # value is optional. If you include a value, it must be between 7 and 30, inclusive. If you do not
      ...
      struct SignRequest
        ...
        # message. The KeyUsage type of the KMS key must be SIGN_VERIFY . To find the KeyUsage of a KMS key,
        # use the DescribeKey operation. To specify a KMS key, use its key ID, key ARN, alias name, or alias
        # ARN. When using an alias name, prefix it with "alias/" . To specify a KMS key in a different Amazon
        # Web Services account, you must use the key ARN or alias ARN. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        ...
        # Specifies the signing algorithm to use when signing the message. Choose an algorithm that is
        ...
        # algorithm. Use RAW for unhashed messages; use DIGEST for message digests, which are already hashed;
        # use EXTERNAL_MU for 64-byte representative μ used in ML-DSA signing as defined in NIST FIPS 204
        # Section 6.2. When the value of MessageType is RAW , KMS uses the standard signing algorithm, which
        # begins with a hash function. When the value is DIGEST , KMS skips the hashing step in the signing
        # algorithm. When the value is EXTERNAL_MU KMS skips the concatenated hashing of the public key hash
        # and the message done in the ML-DSA signing algorithm. Use the DIGEST or EXTERNAL_MU value only when
        # the value of the Message parameter is a message digest. If you use the DIGEST value with an unhashed
        # message, the security of the signing operation can be compromised. When using ECC_NIST_EDWARDS25519
        # KMS keys: ED25519_SHA_512 signing algorithm requires KMS MessageType:RAW ED25519_PH_SHA_512 signing
        # algorithm requires KMS MessageType:DIGEST When the value of MessageType is DIGEST , the length of
        # the Message value must match the length of hashed messages for the specified signing algorithm. When
        # the value of MessageType is EXTERNAL_MU the length of the Message value must be 64 bytes. You can
        # submit a message digest and omit the MessageType or specify RAW so the digest is hashed again while
        # signing. However, this can cause verification failures when verifying with a system that assumes a
        # single hash. The hashing algorithm that Sign uses is based on the SigningAlgorithm value. Signing
        # algorithms that end in SHA_256 use the SHA_256 hashing algorithm. Signing algorithms that end in
      ...
      struct SignResponse
        ...
        # The cryptographic signature that was generated for the message. When used with the supported RSA
        # signing algorithms, the encoding of this value is defined by PKCS #1 in RFC 8017 . When used with
        # the ECDSA_SHA_256 , ECDSA_SHA_384 , or ECDSA_SHA_512 signing algorithms, this value is a DER-encoded
        ...
        # Services CLI, the value is Base64-encoded. Otherwise, it is not Base64-encoded.
        @[JSON::Field(key: "Signature", converter: Aws::Runtime::Base64BytesConverter)]
      ...
      struct TagResourceRequest
        ...
        # Identifies a customer managed key in the account and Region. Specify the key ID or key ARN of the
        ...
        # (null) string. Do not include confidential or sensitive information in this field. This field may be
      ...
      struct UntagResourceRequest
        ...
        # Identifies the KMS key from which you are removing tags. Specify the key ID or key ARN of the KMS
        # key. For example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct UpdateAliasRequest
        ...
        # Identifies the alias that is changing its KMS key. This value must begin with alias/ followed by the
        ...
        # associate an alias with an Amazon Web Services managed key . The KMS key must be in the same Amazon
        # Web Services account and Region as the alias. Also, the new target KMS key must be the same type as
        # the current target KMS key (both symmetric or both asymmetric or both HMAC) and they must have the
        # same key usage. Specify the key ID or key ARN of the KMS key. For example: Key ID:
        # 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
      ...
      struct UpdateCustomKeyStoreRequest
        ...
        # custom key stores with a CustomKeyStoreType of AWS_CLOUDHSM . Enter the cluster ID of the cluster
        # that you used to create the custom key store or a cluster that shares a backup history and has the
        # same cluster certificate as the original cluster. You cannot use this parameter to associate a
        # custom key store with an unrelated cluster. In addition, the replacement cluster must fulfill the
        ...
        # store name must be unique in the Amazon Web Services account. Do not include confidential or
        # sensitive information in this field. This field may be displayed in plaintext in CloudTrail logs and
        ...
        # This parameter is valid only for custom key stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE .
        # You must specify both the AccessKeyId and SecretAccessKey value in the authentication credential,
        # even if you are only updating one value. This parameter doesn't establish or change your
        # authentication credentials on the proxy. It just tells KMS the credential that you established with
        ...
        # proxy uses a Amazon VPC endpoint service to communicate with KMS, specify VPC_ENDPOINT_SERVICE .
        # Otherwise, specify PUBLIC_ENDPOINT . If you change the XksProxyConnectivity to VPC_ENDPOINT_SERVICE
        # , you must also change the XksProxyUriEndpoint and add an XksProxyVpcEndpointServiceName value. If
        ...
        # parameter is valid only for custom key stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE . For
        # external key stores with an XksProxyConnectivity value of PUBLIC_ENDPOINT , the protocol must be
        # HTTPS. For external key stores with an XksProxyConnectivity value of VPC_ENDPOINT_SERVICE , specify
        # https:// followed by the private DNS name associated with the VPC endpoint service. Each external
        ...
        # is valid only for custom key stores with a CustomKeyStoreType of EXTERNAL_KEY_STORE . The value must
        # start with / and must end with /kms/xks/v1 , where v1 represents the version of the KMS external key
        # store proxy API. You can include an optional prefix between the required elements such as / example
      ...
      struct UpdateKeyDescriptionRequest
        ...
        # Updates the description of the specified KMS key. Specify the key ID or key ARN of the KMS key. For
        # example: Key ID: 1234abcd-12ab-34cd-56ef-1234567890ab Key ARN:
        # arn:aws:kms:us-east-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab To get the key ID and
        # key ARN for a KMS key, use ListKeys or DescribeKey .
        @[JSON::Field(key: "KeyId")]
      ...
      struct UpdatePrimaryRegionRequest
        ...
        # key. Specify the key ID or key ARN of a multi-Region primary key. For example: Key ID:
      ...
      struct VerifyRequest
        ...
        # Identifies the asymmetric KMS key that will be used to verify the signature. This must be the same
        # KMS key that was used to generate the signature. If you specify a different KMS key, the signature
        # verification fails. To specify a KMS key, use its key ID, key ARN, alias name, or alias ARN. When
        # using an alias name, prefix it with "alias/" . To specify a KMS key in a different Amazon Web
        ...
        # the value of MessageType is EXTERNAL_MU the length of the Message value must be 64 bytes. You can
        # submit a message digest and omit the MessageType or specify RAW so the digest is hashed again while
        # signing. However, if the signed message is hashed once while signing, but twice while verifying,
        # verification fails, even when the message hasn't changed. The hashing algorithm that Verify uses is
        # based on the SigningAlgorithm value. Signing algorithms that end in SHA_256 use the SHA_256 hashing
        # algorithm. Signing algorithms that end in SHA_384 use the SHA_384 hashing algorithm. Signing
      ...
      # Information about the external key that is associated with a KMS key in an external key store. This
      # element appears in a CreateKey or DescribeKey response only for a KMS key in an external key store.
      # The external key is a symmetric encryption key that is hosted by an external key manager outside of
